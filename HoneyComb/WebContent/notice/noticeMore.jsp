	...
	<div align="center">
		...
		<c:if test="${count > 0}">
			...
			<c:if test="${startPage > pageSize}">
				<a
					href="/semi/notice/noticeMore.notice?pageNum=${startPage - pageSize}">[이전]</a>
			</c:if>

			<c:forEach var="i" begin="${startPage}" end="${endPage}">
				<a href="/semi/notice/noticeMore.notice?pageNum=${i}">[${i}]</a>
			</c:forEach>

			<c:if test="${endPage < pageCount}">
				<a href="/semi/notice/noticeMore.notice?pageNum=${startPage + pageSize}">[다음]</a>
			</c:if>
