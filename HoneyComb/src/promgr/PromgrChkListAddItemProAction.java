...
public class PromgrChkListAddItemProAction implements PromgrFormAction {
	...
	public String requestPro(HttpServletRequest request, HttpServletResponse response) throws Throwable {
		...
		String chklist_title_name = request.getParameter("chklist_title");

		PromgrDBBean dbPro = PromgrDBBean.getInstance();
		int promgr_insert_count = dbPro.insertChkItem(chklist_title_name, article);

		// 해당 view에서 사용할 속성
		request.setAttribute("promgr_insert_count", promgr_insert_count);

		return "/promgr/promgrChkListAddItemPro.jsp";
